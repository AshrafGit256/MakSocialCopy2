...
import { db } from './db';
import { Menu, Home, MessageCircle, User as UserIcon, Bell, Settings, Sun, Moon, Globe, ChevronDown, LayoutGrid, Users } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'messages': 
      case 'groups': return <ChatHub />;
      case 'profile': return <Profile userId={selectedUserId || currentUser?.id} onNavigateBack={() => { setSelectedUserId(null); setView('home'); }} onNavigateToProfile={(id) => setSelectedUserId(id)} onMessageUser={() => setView('messages')} />;
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="sticky top-0 z-[80] bg-[#0d1117]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleTheme} className="p-2 text-slate-500 hover:text-white">{isDark ? <Sun size={20} /> : <Moon size={20} />}</button>
            ...
            {currentUser && <button onClick={() => handleSetView('profile')} className="ml-1"><img src={currentUser.avatar} className={`w-10 h-10 rounded-full border-2 ${view === 'profile' ? 'border-[#10918a]' : 'border-white/5'} bg-white object-cover`} alt="Profile" /></button>}
          </div>
        </header>
