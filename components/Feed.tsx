...
import { 
  ...
  FileText, ExternalLink, Type, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight,
  List, ListOrdered, Link as LinkIcon, Table, Palette, Heading1, Heading2, Quote,
  Smile, MoreHorizontal, Check
} from 'lucide-react';
...
const Feed: React.FC<{ collegeFilter?: College, targetPostId?: string | null, onClearTarget?: () => void }> = ({ collegeFilter, targetPostId, onClearTarget }) => {
  ...
  const insertLink = () => {
    const url = prompt("Enter Protocol URL (e.g., https://mak.ac.ug):");
    if (url) exec('createLink', url);
  };

  const insertTable = () => {
    const rows = prompt("Enter row count:", "2");
    const cols = prompt("Enter column count:", "2");
    if (rows && cols) {
      let tableHtml = `<table style="width:100%; border-collapse: collapse; margin: 10px 0; border: 1px solid var(--border-color);">`;
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Verify safety and category for this rich-text student post. Respond ONLY with JSON: {"category": "Academic"|"Social"|"Career", "isSafe": boolean}. Content: "${content.replace(/<[^>]*>?/gm, '')}"`,
    ...
    <div className="max-w-[1440px] mx-auto pb-32 lg:px-12 lg:py-8">
      {!collegeFilter && (
        <div className="sticky top-0 lg:top-[3.75rem] z-[75] bg-[var(--bg-primary)]/80 backdrop-blur-xl border-b border-[var(--border-color)]">
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-3 px-4">
            <button onClick={() => setActiveTab('Global')} className={`px-5 py-2 rounded-[4px] text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${activeTab === 'Global' ? 'bg-indigo-600 text-white border-transparent' : 'text-slate-500 bg-[var(--bg-secondary)] border-[var(--border-color)]'}`}>Global Pulse</button>
            {['COCIS', 'CEDAT', 'CHUSS', 'CONAS', 'CHS', 'CAES', 'COBAMS', 'CEES', 'LAW'].map(c => (
              <button key={c} onClick={() => setActiveTab(c as College)} className={`flex items-center gap-2 px-5 py-2 rounded-[4px] text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${activeTab === c ? 'bg-indigo-600 text-white border-transparent' : 'text-slate-500 bg-[var(--bg-secondary)] border-[var(--border-color)]'}`}>{c}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-10 mt-6">
         <div className="lg:col-span-8 space-y-6 p-4 lg:p-0">
            {/* ADVANCED RICH TEXT COMPOSER (Summernote Style) */}
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[6px] shadow-sm overflow-hidden">
               ...
               <div className="bg-slate-50 dark:bg-white/5 border-b border-[var(--border-color)] px-4 py-2 flex flex-wrap items-center gap-1">
                  <div className="flex items-center gap-0.5 border-r border-[var(--border-color)] pr-2 mr-2">
                  ...
                  <div className="flex items-center gap-0.5 border-r border-[var(--border-color)] pr-2 mr-2">
                     <button onClick={() => exec('justifyLeft')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Align Left"><AlignLeft size={15}/></button>
                     <button onClick={() => exec('justifyCenter')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Align Center"><AlignCenter size={15}/></button>
                     <button onClick={() => exec('justifyRight')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Align Right"><AlignRight size={15}/></button>
                  </div>
                  <div className="flex items-center gap-0.5 border-r border-[var(--border-color)] pr-2 mr-2">
                     <button onClick={() => exec('insertUnorderedList')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Bullets"><List size={15}/></button>
                     <button onClick={() => exec('insertOrderedList')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Numbers"><ListOrdered size={15}/></button>
                  </div>
                  <div className="flex items-center gap-0.5 border-r border-[var(--border-color)] pr-2 mr-2">
                     <button onClick={insertLink} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Insert Link"><LinkIcon size={15}/></button>
                     <button onClick={insertTable} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Insert Table"><Table size={15}/></button>
                     <button onClick={() => exec('hiliteColor', '#fef08a')} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded" title="Highlight"><Palette size={15}/></button>
                  </div>
                  <div className="relative group">
                    ...
                    <div className="absolute top-full left-0 mt-1 bg-white dark:bg-slate-800 border border-[var(--border-color)] rounded shadow-xl z-[100] w-40 opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all">
                      {fonts.map(f => (
               ...
               <div className="p-6">
                  <div className="flex gap-4">
                     <img src={user.avatar} className="w-10 h-10 rounded-[4px] border border-[var(--border-color)] bg-white shrink-0" />
                     <div className="flex-1 space-y-4">
                       <div 
                         ...
                         data-placeholder={`Broadcast creative signal to ${activeCollege || 'the Hill'}...`}
                       />
                       {selectedImage && (
                  ...
                  <div className="flex justify-between items-center mt-6 pt-4 border-t border-[var(--border-color)]">
                    ...
                    <button onClick={handleCreatePost} disabled={isAnalyzing} className="bg-indigo-600 text-white px-8 py-2.5 rounded-[4px] font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-md active:scale-95">
                     ...
                     <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                           ...
                           <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[6px] shadow-sm overflow-hidden transition-all hover:border-indigo-500/30">
                              <div className="p-5 lg:p-6 space-y-4 prose dark:prose-invert max-w-none text-[14px]" style={{ fontFamily: post.customFont }}>
                                 <div dangerouslySetInnerHTML={{ __html: post.content }} className="rich-content" />
                                 {post.images?.[0] && <img src={post.images[0]} className="w-full object-cover max-h-[450px] rounded-[4px]" />}
                              </div>
         ...
         <aside className="hidden lg:block lg:col-span-4 space-y-8 sticky top-20 h-fit">
            <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[6px] p-6 shadow-sm">
               <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center gap-2"><TrendingUp size={16} className="text-indigo-600"/> Featured Channels</h3>
               <div className="space-y-4">
                  {['#ResearchProtocol', '#GuildInauguration', '#BlockchainMakerere', '#LiteratureHub'].map(tag => (
      ...
      <style>{`
        ...
        .rich-content h1 { font-size: 1.5rem; font-weight: 900; margin-bottom: 0.5rem; color: var(--text-primary); text-transform: uppercase; }
        .rich-content h2 { font-size: 1.25rem; font-weight: 800; margin-bottom: 0.5rem; color: var(--text-primary); }
        .rich-content p { margin-bottom: 0.5rem; }
        .rich-content blockquote { border-left: 4px solid #6366f1; padding-left: 1rem; font-style: italic; color: #64748b; margin: 1rem 0; }
        .rich-content ul, .rich-content ol { padding-left: 1.5rem; margin: 1rem 0; }
        .rich-content ul { list-style-type: disc; }
        .rich-content ol { list-style-type: decimal; }
        .rich-content table td, .rich-content table th { border: 1px solid var(--border-color); padding: 8px; }
      `}</style>
    </div>
  );
};
