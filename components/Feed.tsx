...
const VerifiedBadge: React.FC<{ role?: AuthorityRole }> = ({ role }) => {
  ...
  const styles: Record<AuthorityRole, { bg: string, text: string }> = {
    ...
    'Lecturer': { bg: 'bg-black', text: 'text-white' },
    'Chairperson': { bg: 'bg-emerald-500', text: 'text-white' },
    'GRC': { bg: 'bg-emerald-600', text: 'text-white' },
    'Student Leader': { bg: 'bg-emerald-400', text: 'text-white' }
  };
  const style = styles[role] || styles['Student Leader'];
  return (
    <div className={`${style.bg} ${style.text} p-0.5 rounded-full shadow-sm flex items-center justify-center`} title={role}>
...
const Feed: React.FC<{ collegeFilter?: College, targetPostId?: string | null, onClearTarget?: () => void }> = ({ collegeFilter, targetPostId, onClearTarget }) => {
  ...
  const isCollegeAdmin = activeCollege ? user.email?.toLowerCase() === `admin.${activeCollege.toLowerCase()}@mak.ac.ug` : false;
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const newPost: Post = {
        ...
        authorAuthority: isCollegeAdmin ? 'Administrator' : (isSuperAdmin ? 'Super Admin' : undefined),
  ...
  const handleDelete = (pid: string) => {
    if (confirm("Execute Veto: Remove this content?")) {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-8 pb-32">
           ...
           <div className="p-8 -mt-24 relative z-10 space-y-8">
              <div className="flex flex-col md:flex-row items-end justify-between gap-6">
                 ...
                 <div className="flex items-center gap-3">
                    ...
                    {isCollegeAdmin && (
                       <button onClick={() => setShowManageModal(true)} className="p-4 bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all">
                          <Edit3 size={20}/>
                       </button>
                    )}
      ...
      {/* College Admin Registry Modal */}
      {showManageModal && activeCollege && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-lg">
           <div className="glass-card w-full max-w-2xl bg-[var(--sidebar-bg)] border-[var(--border-color)] shadow-2xl overflow-hidden flex flex-col h-[80vh]">
              <div className="p-8 border-b border-[var(--border-color)] flex justify-between items-center bg-[var(--bg-secondary)]">
                 <div>
                    <h2 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter uppercase">Registry Management</h2>
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mt-1">Authorized Node: {activeCollege}</p>
                 </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button onClick={() => {
                       const name = prompt("Official Name:");
                       const role = prompt("Role (Lecturer / GRC / Chairperson / Student Leader):") as AuthorityRole;
                    ...
                    }} className="flex items-center justify-center gap-3 p-6 bg-indigo-600 text-white rounded-[2rem] font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-600/30">
                       <UserPlus size={20}/> Add Authority Node
                    </button>
                    <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-[2rem] flex flex-col items-center justify-center text-center">
                       <Shield size={24} className="text-rose-500 mb-2"/>
                       <p className="text-[8px] font-black text-rose-500 uppercase tracking-widest">Active Veto Protocol</p>
                       <p className="text-[10px] font-bold text-rose-800 dark:text-rose-300">Enabled for all node signals</p>
                    </div>
                 </div>

                 <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Users size={14}/> Existing Registry</h4>
                    <div className="space-y-3">
                       {currentStats?.leadership.map(m => (
                          <div key={m.id} className="flex items-center justify-between p-4 bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-color)]">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   ...
                                   <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{m.role} • {m.email}</p>
                                </div>
                             </div>
                             <button onClick={() => {
                                ...
                                db.updateCollegeLeadership(activeCollege, newList);
                                setCollegeStats(db.getCollegeStats());
                             }} className="p-3 text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all"><Trash2 size={18}/></button>
                          </div>
                       ))}
                    </div>
                 </div>
