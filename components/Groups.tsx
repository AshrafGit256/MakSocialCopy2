      ...
      <aside className="hidden lg:flex w-80 border-r border-[var(--border-color)] flex-col bg-[var(--sidebar-bg)] shrink-0 z-20">
        <div className="p-6 border-b border-[var(--border-color)] space-y-5 bg-white/50 dark:bg-black/20">
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="p-1.5 bg-indigo-600/10 text-indigo-600 border border-indigo-600/20 rounded hover:bg-indigo-600 hover:text-white transition-all shadow-sm active:scale-90"
            >
              <Plus size={16} />
            </button>
          </div>
          <div className="relative group">
            ...
            <input 
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded py-2.5 pl-9 pr-4 text-[10px] font-bold uppercase outline-none focus:border-indigo-600 transition-all shadow-inner"
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar py-2">
          {filteredGroups.map(g => (
            ...
            >
              {activeGroupId === g.id && <div className="absolute inset-y-0 left-0 w-1 bg-indigo-600 animate-pulse"></div>}
              <div className="relative shrink-0">
                <img src={g.image} className="w-11 h-11 rounded border border-[var(--border-color)] object-cover bg-white grayscale group-hover:grayscale-0 transition-all shadow-sm" />
                {g.isOfficial && (
        ...
        <div className="p-6 border-t border-[var(--border-color)] bg-slate-50/50 dark:bg-black/20 space-y-4">
           ...
           <div className="h-1 w-full bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-indigo-600 w-[89%] animate-pulse"></div>
           </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col bg-[var(--bg-primary)] min-w-0 h-full relative">
        {activeGroup ? (
          <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                 ...
                 <div className="flex items-center gap-3">
                    <div className="bg-[var(--bg-secondary)] p-1 rounded border border-[var(--border-color)] flex shadow-inner">
                       <button onClick={() => setViewMode('terminal')} className={`px-4 py-1.5 rounded text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === 'terminal' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-700'}`}>
                          <Terminal size={12}/> Terminal
                       </button>
                       <button onClick={() => setViewMode('architecture')} className={`px-4 py-1.5 rounded text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === 'architecture' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-700'}`}>
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 flex flex-col relative overflow-hidden bg-slate-50 dark:bg-[#080808]">
                {viewMode === 'terminal' ? (
                  <>
                    {/* CHAT STREAM (TACTICAL ALIGNMENT) */}
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 no-scrollbar" style={{ backgroundImage: 'linear-gradient(var(--border-color) 0.5px, transparent 0.5px), linear-gradient(90deg, var(--border-color) 0.5px, transparent 0.5px)', backgroundSize: '40px 40px' }}>
                      
                      {/* Neural Snapshot Banner */}
                      {neuralSnapshot && (
                        <div className="bg-slate-700 text-white rounded-xl p-8 mb-10 animate-in slide-in-from-top-4 relative overflow-hidden group shadow-2xl">
                            ...
                            <div className={`flex max-w-[85%] sm:max-w-[70%] gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                              ...
                              {!isMe && (
                                <div className="shrink-0">
                                   <img src={msg.authorAvatar} className="w-8 h-8 rounded border border-[var(--border-color)] object-cover bg-white mt-1" />
                                </div>
                              )}

                              <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} space-y-1`}>
                                {!isMe && (
                                  <span className="text-[9px] font-black uppercase text-slate-500 ml-1 tracking-widest">{msg.author}</span>
                                )}
                                
                                <div className={`p-4 rounded shadow-sm relative group ${
                                  isMe 
                                  ? 'bg-[#475569] text-white rounded-tr-none' 
                                  ...
                                  {msg.attachment && (
                                    <div className="mb-3">
                                      {msg.attachment.type === 'image' ? (
                                        ...
                                        <div className={`flex items-center gap-3 p-3 rounded border ${isMe ? 'bg-black/20 border-white/10' : 'bg-slate-50 dark:bg-black/20 border-[var(--border-color)]'}`}>
                                          ...
                                          <div className="min-w-0">
                                            ...
                                            <p className="text-[8px] opacity-60 font-bold uppercase tracking-widest">Protocol_Asset_v1</p>
                                          </div>
                                  ...
                                  <div className={`flex items-center gap-2 mt-2 justify-end ${isMe ? 'text-slate-300' : 'text-slate-400'}`}>
                                    ...
                                    <span className="text-[7px] font-mono opacity-40 group-hover:opacity-100 transition-opacity">commit_{SHA_GEN().slice(0,4)}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                       ...
                       {isMember ? (
                         <div className="bg-[var(--bg-primary)] border border-[var(--border-color)] rounded shadow-2xl transition-all overflow-hidden">
                            ...
                            <div className="px-6 py-3 border-b border-[var(--border-color)] bg-slate-50/80 dark:bg-black/40 flex items-center justify-between">
                               <div className="flex items-center gap-4">
                                  <button onClick={() => fileInputRef.current?.click()} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded text-slate-400 hover:text-indigo-600 transition-all"><Paperclip size={18}/></button>
                                  <div className="w-px h-6 bg-[var(--border-color)]"></div>
                                  <button onClick={handleNeuralSync} disabled={isSummarizing} className="px-4 py-1.5 hover:bg-indigo-600/10 rounded text-indigo-600 transition-all flex items-center gap-2 disabled:opacity-30">
                               ...
                               <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Link_Stable</span>
                               </div>
                            </div>
                         ...
                         <div className="p-16 border border-dashed border-amber-600/40 bg-amber-500/5 rounded flex flex-col items-center gap-8 text-center shadow-inner">
                            ...
                            <div className="space-y-3">
                               <h3 className="text-2xl font-black uppercase italic tracking-tighter text-amber-600">Access_Protocol_Denied</h3>
                            ...
                            <button onClick={() => handleJoin(activeGroup.id)} className="px-16 py-5 bg-amber-600 text-white rounded text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-amber-600/20 active:scale-95 transition-all">Synchronize Node</button>
                         </div>
                       )}
                    </div>
                  </>
                  ...
                  <div className="flex-1 overflow-y-auto p-12 space-y-12 no-scrollbar animate-in fade-in zoom-in-98 duration-500">
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        {/* Activity Heatmap */}
                        <div className="bg-white dark:bg-[#0b0f1a] border border-[var(--border-color)] p-10 rounded shadow-2xl space-y-10">
                           <div className="flex items-center justify-between">
                              ...
                              <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded text-[8px] font-black uppercase text-emerald-500">High Frequency</div>
                           </div>
                           ...
                           <div className="grid grid-cols-3 gap-8">
                              <div className="text-center p-6 bg-slate-50 dark:bg-black/40 rounded border border-[var(--border-color)]">
                                 <p className="text-3xl font-black text-indigo-600">{activeGroup.memberIds.length}</p>
                                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Indexed_Nodes</p>
                              </div>
                              <div className="text-center p-6 bg-slate-50 dark:bg-black/40 rounded border border-[var(--border-color)]">
                                 <p className="text-3xl font-black text-indigo-600">{activeGroup.messages.length}</p>
                        ...
                        <div className="bg-white dark:bg-[#0b0f1a] border border-[var(--border-color)] p-10 rounded shadow-2xl space-y-10">
                           ...
                           <div className="space-y-6">
                              {[1, 2, 3].map(i => (
                                 <div key={i} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-black/40 border border-[var(--border-color)] rounded hover:border-indigo-600 transition-all cursor-pointer group">
                                    <div className="flex items-center gap-6">
                                       <div className="p-4 bg-white dark:bg-slate-900 border border-[var(--border-color)] rounded group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm">
                                          <FileText size={20} />
                                       </div>
                                       <div>
                                          <p className="text-[13px] font-black uppercase tracking-tight">Academic_Log_v{i}.log</p>
                           ...
                           <button className="w-full py-5 border-2 border-dashed border-[var(--border-color)] rounded text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 hover:border-indigo-600 hover:text-indigo-600 transition-all flex items-center justify-center gap-4 shadow-inner">
                     ...
                     {/* Strategic Brief */}
                     <div className="p-12 bg-indigo-600 rounded text-white relative overflow-hidden shadow-2xl">
                        ...
                        <div className="relative z-10 space-y-10">
                           ...
                           <p className="text-xl font-medium leading-relaxed italic text-indigo-100 max-w-4xl border-l-4 border-white/20 pl-10">
                              "{activeGroup.description}"
                           </p>
                           <div className="flex flex-wrap gap-6 pt-6">
                              <button className="px-12 py-5 bg-white text-indigo-600 rounded font-black text-[11px] uppercase tracking-[0.4em] hover:bg-slate-100 transition-all active:scale-95 shadow-2xl">Verify Security</button>
              ...
              <aside className="hidden xl:flex w-80 border-l border-[var(--border-color)] flex-col bg-[var(--sidebar-bg)] p-8 space-y-12 z-20 overflow-y-auto no-scrollbar">
                 ...
                 <div className="pt-10 border-t border-[var(--border-color)] space-y-6">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] flex items-center gap-3">
                       <History size={16} className="text-indigo-600"/> Cluster_History
                    </h4>
                    <div className="space-y-6">
                       {[1, 2, 3, 4].map(i => (
                          <div key={i} className="relative pl-6 border-l-2 border-indigo-600/20 space-y-2 group hover:border-indigo-600 transition-all">
                             ...
                             <p className="text-[9px] text-slate-500 font-medium italic leading-none">Logic indexed @ {i * 4}h ago</p>
                          </div>
                       ))}
                    </div>
                 </div>

                 <div className="mt-auto p-6 bg-slate-900 dark:bg-black rounded border border-slate-800 space-y-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-12">
             <div className="relative group">
                <div className="absolute -inset-10 bg-indigo-600/10 blur-[60px] rounded-full group-hover:bg-indigo-600/20 transition-all duration-1000"></div>
                <div className="w-48 h-48 bg-indigo-600/5 rounded-full flex items-center justify-center border-2 border-dashed border-indigo-600/30 shadow-inner transition-all group-hover:border-indigo-600 group-hover:scale-110">
                   <Users size={100} className="text-indigo-600/20 group-hover:text-indigo-600 transition-colors" />
                </div>
             </div>
             ...
             <div className="flex gap-8">
                <button onClick={() => setIsCreating(true)} className="px-14 py-6 bg-indigo-600 text-white rounded font-black text-[12px] uppercase tracking-[0.5em] shadow-2xl shadow-indigo-600/30 active:scale-95 transition-all flex items-center gap-4 hover:bg-indigo-700"><Plus size={24}/> Initialize Hub</button>
           ...
           <div className="bg-[var(--bg-primary)] w-full max-w-xl p-12 rounded shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] space-y-12 border border-[var(--border-color)] relative">
              ...
              <div className="space-y-10">
                 <div className="space-y-4 group">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] ml-1 group-focus-within:text-indigo-600 transition-colors">Cluster_Identifier (Name)</label>
                    <input className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded p-6 text-sm font-bold outline-none focus:border-indigo-600 transition-all shadow-inner uppercase tracking-tighter" value={createForm.name} onChange={e => setCreateForm({...createForm, name: e.target.value})} placeholder="e.g. COCIS_Neural_Net" />
                 </div>
                 <div className="space-y-4">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] ml-1">Sector_Allocation</label>
                    <select className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded p-6 text-sm font-bold outline-none cursor-pointer hover:border-indigo-600 transition-all appearance-none uppercase tracking-tighter" value={createForm.category} onChange={e => setCreateForm({...createForm, category: e.target.value})}>
                 ...
                 <button onClick={handleCreateGroup} className="w-full bg-indigo-600 hover:bg-indigo-700 py-6 rounded text-white font-black text-xs uppercase tracking-[0.6em] shadow-2xl shadow-indigo-600/30 active:scale-95 transition-all">Commit Cluster to Registry</button>
              </div>
           </div>
        </div>
      )}
