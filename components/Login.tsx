...
const Login: React.FC<LoginProps> = ({ onLogin, onSwitchToRegister }) => {
  ...
  return (
    <div className="flex h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-hidden selection:bg-slate-700 selection:text-white font-sans transition-colors duration-500">
      {/* LEFT TACTICAL PANEL - RETAINS TACTICAL LOOK BUT THEME AWARE */}
      <div className="hidden lg:block w-1/2 relative bg-[#05080c] border-r border-[var(--border-color)]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent p-20 flex flex-col justify-end">
           <div className="space-y-8 max-w-lg">
              <div className="flex items-center gap-4">
                 ...
                 <div className="space-y-1">
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Registry Encryption: AES-256</p>
                 </div>
              </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-[var(--bg-primary)] relative">
        <div className="absolute top-10 right-10 flex items-center gap-4">
           ...
           <button 
             ...
             className="px-4 py-2 border border-[var(--border-color)] rounded-[2px] text-[10px] font-black uppercase tracking-widest text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-all"
           >
        ...
        <div className="w-full max-w-md space-y-10">
          <div className="space-y-3">
             ...
             <h2 className="text-4xl font-black text-[var(--text-primary)] uppercase tracking-tight">Identity Handshake.</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-2 group">
                ...
                <div className="relative">
                   <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[var(--brand-color)] transition-colors" size={18} />
                   <input 
                     type="email" 
                     value={email} 
                     onChange={(e) => setEmail(e.target.value)} 
                     placeholder="student@mak.ac.ug" 
                     className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-[2px] py-4 pl-12 pr-6 text-sm font-bold text-[var(--text-primary)] outline-none focus:border-slate-400 focus:ring-4 focus:ring-slate-500/5 transition-all" 
                     required 
                   />
                </div>
              </div>
              <div className="space-y-2 group">
                ...
                <div className="relative">
                   <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[var(--brand-color)] transition-colors" size={18} />
                   <input 
                     type="password" 
                     placeholder="••••••••" 
                     className="w-full bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-[2px] py-4 pl-12 pr-6 text-sm font-bold text-[var(--text-primary)] outline-none focus:border-slate-400 focus:ring-4 focus:ring-slate-500/5 transition-all" 
            ...
            <button 
              type="submit" 
              className="w-full bg-[var(--brand-color)] text-white font-black py-5 rounded-[2px] text-xs uppercase tracking-[0.2em] transition-all shadow-xl hover:brightness-110 active:scale-95 flex items-center justify-center gap-3"
            >
