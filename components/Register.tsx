  ...
  return (
    <div className="flex h-screen bg-[var(--bg-primary)] overflow-hidden selection:bg-slate-500 selection:text-white">
      <div className="hidden lg:block w-1/2 relative bg-[#0f172a]">
        <img src="https://images.unsplash.com/photo-1523580846011-d3a5bc25702b?auto=format&fit=crop&w=1200" className="absolute inset-0 w-full h-full object-cover brightness-50 transition-all duration-1000" alt="Graduation Spirit" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0f172a] via-[#0f172a]/60 to-transparent p-24 flex flex-col justify-end space-y-10">
           <h1 className="text-7xl font-black text-white italic tracking-tighter uppercase leading-[0.9]">Define <br /> Your <span className="text-slate-500">Node</span>.</h1>
           <p className="text-slate-300 text-xl max-w-md font-medium leading-relaxed italic border-l-4 border-slate-600 pl-8">"Register your identity within the hill's collective network. Every node contributes to the research pulse."</p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white dark:bg-[#0f172a] relative overflow-y-auto no-scrollbar">
        <div className="absolute top-10 right-10">
           <button onClick={onSwitchToLogin} className="text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-slate-600">Existing Identity? Sign In</button>
        </div>
        <div className="w-full max-w-md py-12 space-y-12">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-600/10 rounded-lg text-slate-600 text-[8px] font-black uppercase tracking-widest">Onboarding Sequence</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-6">
               <div className="space-y-2 group">
                ...
                <input type="text" placeholder="e.g. Ashraf G." className="w-full bg-slate-50 dark:bg-white/5 border border-[var(--border-color)] rounded-2xl py-5 px-6 text-sm font-bold text-[var(--text-primary)] outline-none focus:ring-4 focus:ring-slate-600/10 focus:border-slate-600 transition-all" value={name} onChange={e => setName(e.target.value)} required />
              </div>
              <div className="space-y-2 group">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Wing Credential (Email)</label>
                <input type="email" placeholder="name@mak.ac.ug" className="w-full bg-slate-50 dark:bg-white/5 border border-[var(--border-color)] rounded-2xl py-5 px-6 text-sm font-bold text-[var(--text-primary)] outline-none focus:ring-4 focus:ring-slate-600/10 focus:border-slate-600 transition-all" value={email} onChange={e => setEmail(e.target.value)} required />
              </div>
            ...
            <button type="submit" className="w-full bg-slate-600 hover:bg-slate-700 text-white font-black py-6 rounded-[2rem] text-xs uppercase tracking-[0.3em] transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3">Commit Identity <ArrowRight size={18} /></button>
          </form>
