...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setView, isAdmin, onLogout, isOpen, onSearchToggle }) => {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-[2001] w-64 bg-white border-r border-[var(--border-color)] flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex-1 overflow-y-auto no-scrollbar">
        ...
        <nav className="px-3 space-y-1">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-3 rounded-full transition-all group ${activeView === item.id ? 'text-[var(--brand-color)] font-bold' : 'text-slate-900 hover:bg-slate-100'}`}
            >
              <div className={activeView === item.id ? 'text-[var(--brand-color)]' : 'text-slate-900'}>
                {item.icon}
              </div>
              <span className="text-[20px] font-bold tracking-tight">{item.label}</span>
            </button>
          ))}
        </nav>
        ...
        {isAdmin && (
          <div className="mt-8 pt-8 px-3 border-t border-[var(--border-color)]">
            <button onClick={() => setView('admin')} className={`w-full flex items-center gap-4 px-5 py-3 rounded-full transition-all ${activeView === 'admin' ? 'text-[var(--brand-color)] font-bold' : 'text-slate-900 hover:bg-slate-100'}`}>
              <Cpu size={22} />
              <span className="text-[20px] font-bold tracking-tight">Admin Terminal</span>
            </button>
          </div>
        )}
      </div>
